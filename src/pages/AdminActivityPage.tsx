...
const entityLabels = { user: 'usuário', unit: 'unidade', room: 'sala', booking: 'agendamento', task: 'tarefa', hours_plan: 'plano de horas', backup: 'backup' };
...
export function AdminActivityPage() {
  ...
  return <main className="governance-page">
    ...
    <section className="governance-card governance-card--wide"><div className="governance-toolbar"><div><h2>Atividades recentes</h2><p>{filtered.length} registros encontrados</p></div><input type="search" placeholder="Buscar pessoa, item ou ID" value={query} onChange={(event) => { setQuery(event.target.value); setLimit(30); }} /></div>
      <div className="activity-list">{filtered.slice(0, limit).map((log) => <article key={log.id}><span className="activity-dot" /><div><strong>{actionLabels[log.action]} {entityLabels[log.entity]}</strong><p>{log.actorName ?? 'Sistema'} · <code title={log.entityId}>{log.entityId}</code></p></div><time dateTime={log.occurredAt}>{new Date(log.occurredAt).toLocaleString('pt-BR')}</time></article>)}{!filtered.length && <p className="governance-empty">Nenhuma atividade encontrada.</p>}</div>
