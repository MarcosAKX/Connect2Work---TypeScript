...
export function AdminBookingsPage() {
  ...
  return <main className="admin-bookings-page">
    ...
    <section className="admin-bookings-stats" aria-label="Resumo dos agendamentos" aria-busy={data.isLoading}>{stats.map(({ label, value, icon: Icon, tone, action }) => { const content = <><span><Icon width="19" height="19" />{label}</span><strong>{data.isLoading ? '—' : value}</strong></>; return action ? <button type="button" key={label} className={`admin-bookings-stat tone-${tone}`} onClick={action}>{content}<small>Aplicar filtro de hoje</small></button> : <article key={label} className={`admin-bookings-stat tone-${tone}`}>{content}</article>; })}</section>
          ...
          <table className="admin-bookings-table">
            ...
            <tbody>{data.bookings.map((item) => {
              ...
              return <tr key={item.booking.id} className={[item.adminStatus === 'pending' ? 'is-pending' : '', isAwaitingCheckIn ? 'is-awaiting-checkin' : ''].filter(Boolean).join(' ') || undefined}>
                ...
                <td data-label="Pagamento">{paymentStatus === 'pending' && item.adminStatus !== 'cancelled' ? <button type="button" className="admin-bookings-payment is-pending is-clickable" onClick={() => { data.clearMutationError(); setConfirmingPayment(item); }} disabled={data.isSaving} title="Confirmar recebimento">Pendente</button> : <span className={`admin-bookings-payment is-${paymentStatus}`}>{paymentStatus === 'completed' ? 'Concluído' : 'Pendente'}</span>}</td>
