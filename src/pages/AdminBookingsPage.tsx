...
export function AdminBookingsPage() {
  ...
  return <main className="admin-bookings-page">
    ...
    <section className="admin-bookings-list" aria-labelledby="admin-bookings-list-title"><header><div><h2 id="admin-bookings-list-title">Lista de Agendamentos</h2><p>{data.bookings.length} agendamentos encontrados</p></div><div className="admin-bookings-list-summary"><span><strong>{data.stats.confirmed}</strong> confirmados</span><span><strong>{data.stats.cancelled}</strong> cancelados</span>{user?.role === 'admin' && <span><strong>{money.format(data.stats.revenue)}</strong> recebidos</span>}</div></header><div className="admin-bookings-filters" role="search" aria-label="Filtros de agendamentos"><label className="admin-bookings-search"><span className="admin-filter-label">Buscar</span><input value={data.search} onChange={(event) => data.setSearch(event.target.value)} placeholder="Cliente, sala, unidade ou ID" /></label><label><span className="admin-filter-label">Unidade</span><select value={data.unitId} onChange={(event) => data.setUnitId(event.target.value)}><option value="">Todas unidades</option>{data.units.map((unit) => <option key={unit.id} value={unit.id}>{unit.name}</option>)}</select></label><label><span className="admin-filter-label">Status</span><select value={data.status} onChange={(event) => data.setStatus(event.target.value as AdminBookingStatusFilter)}><option value="">Todos</option><option value="confirmed">Confirmado</option><option value="pending">Pendente</option><option value="cancelled">Cancelado</option></select></label><label className="admin-bookings-date"><span className="admin-filter-label">De</span><input type="date" value={data.dateFrom} max={data.dateTo || undefined} onChange={(event) => data.setDateFrom(event.target.value)} /></label><label className="admin-bookings-date"><span className="admin-filter-label">Até</span><input type="date" value={data.dateTo} min={data.dateFrom || undefined} onChange={(event) => data.setDateTo(event.target.value)} /></label><button type="button" className="admin-bookings-show-all" onClick={data.showAllDates}>Mostrar todos</button></div>
      {data.isLoading ? (
